...
void add_vec(const float *a, const float *b, float *c, size_t length)
{
    while (length > 0) {
        ...
        vfloat32m2_t va = __riscv_vle32_v_f32m2(a, vl); // load 8 elements from a
        vfloat32m2_t vb = __riscv_vle32_v_f32m2(b, vl); // load 8 elements from b
        vfloat32m2_t vc = __riscv_vfadd_vv_f32m2(va, vb, vl); // c = a + b
        __riscv_vse32_v_f32m2(c, vc, vl); // store 8 elements

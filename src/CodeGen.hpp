...
class CodeBlock{
    public:
        ...
        std::string ptrName;  // for size_t ptr
        ...
        void initializeCodeBlock() { 
            ...
            blockName = "test"; blockNum++;
            vlName = "vl";
            ptrName = "ptr";
            maskName = "vmask";
            for(size_t i=0; i<dataLen; ++i){
                std::string val = getAScalarRandom("mask");
                if(val == std::to_string(0)) { vmask.push_back(false); }
                else if(val == std::to_string(1)) { vmask.push_back(true); }
                else{
        ...
        std::vector<std::string> load_vle(std::string type, std::string regName, std::string loadVar, bool mask = false);
        std::vector<std::string> store_vse(std::string type, std::string regName, bool mask = false);
        // Vector Strided Load and store Intrinsics
        std::vector<std::string> load_vlse(std::string type, std::string regName, std::string loadVar, bool mask = false);
        std::vector<std::string> store_vsse(std::string type, std::string regName, bool mask = false);
        // Vector Indexed Load and Store Intrinsics
        std::vector<std::string> load_vloxei(std::string type, std::string regName, std::string loadVar, bool mask = false);
        std::vector<std::string> load_vluxei(std::string type, std::string regName, std::string loadVar, bool mask = false);
        std::vector<std::string> store_vsoxei(std::string type, std::string regName, bool mask = false);
        std::vector<std::string> store_vsuxei(std::string type, std::string regName, bool mask = false);
